...
-- 1. Создаем пользователя 'afisha_prod_user' с паролем 'your_password' (пароль нужно придумать свой).
CREATE ROLE afisha_prod_user WITH LOGIN PASSWORD 'your_password';

-- 2. Создаем базу данных 'afisha' и сразу делаем 'afisha_prod_user' ее владельцем.
CREATE DATABASE afisha WITH OWNER afisha_prod_user;
*/


-- Дальше все команды нужно выполнять, подключившись к базе 'afisha' под пользователем 'afisha_prod_user'.
...
ALTER TABLE public.films OWNER TO afisha_prod_user;
...
ALTER TABLE public.schedules OWNER TO afisha_prod_user;
...
ALTER TABLE public.orders OWNER TO afisha_prod_user;